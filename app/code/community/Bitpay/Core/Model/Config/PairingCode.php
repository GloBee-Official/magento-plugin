...
class Bitpay_Core_Model_Config_PairingCode extends Mage_Core_Model_Config_Data
{
    ...
    public function save()
    {
        ...
        \Mage::helper('bitpay')->debugData('[INFO] In Bitpay_Core_Model_Config_PairingCode::save(): attempting to pair with GloBee with pairing code ' . $pairingCode);
        ...
        try {
            ...
            \Mage::getSingleton('core/session')->addError('There was an error while trying to pair with GloBee using the pairing code '.$pairingCode.'. Please try again or enable debug mode and send the "payment_bitpay.log" file to support@bglobee.com for more help.');

            return;
        }

        \Mage::getSingleton('core/session')->addSuccess('Pairing with GloBee was successful.');
    }
}
